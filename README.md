...
- используется парсер `reshak`;
- результаты сохраняются в папку `output`;
- обрабатываются две стартовые страницы;
- используется список прокси;
- выполняется полный парсинг в режиме `all`;
- для каждого запроса допускается до `5` попыток;
- таймаут запроса составляет `5` секунд;
- включена обработка изображений;
- включен вывод логов в консоль.